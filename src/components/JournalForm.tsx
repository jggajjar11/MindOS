...
export default function JournalForm({ profile, onEntryAdded, entries, onDeleteEntry }: JournalFormProps) {
  ...
  const handleAnalyzeAndSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!text.trim()) {
      setError("Please write down what is on your mind. Even a brief sentence helps standard analysis.");
      return;
    }
    setError("");
    setIsAnalyzing(true);
        ...
        >
          ...
          <form onSubmit={handleAnalyzeAndSubmit} className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-indigo-300 font-medium mb-3">{getMoodText(moodScore)}</p>
            ...
            {/* Markdown Text Area */}
            <div>
              <label htmlFor="journal-textarea" className="block text-xs font-mono uppercase tracking-wider text-zinc-400 mb-2">
            ...
            {error && (
              <div className="p-3 bg-red-950/20 border border-red-900/30 rounded-xl text-xs text-red-300" id="journal-input-error">
                {error}
              </div>
            )}
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 bg-indigo-950/20 rounded-xl border border-indigo-500/10">
                      <p className="text-[10px] text-indigo-400 font-mono uppercase">Assessed Emotion</p>
                      <p className="text-base font-bold text-white mt-1 capitalize">{lastAnalysis.emotion}</p>
                    </div>
                    <div className="p-3 bg-white/5 rounded-xl border border-white/5">
                      <p className="text-[10px] text-sky-400 font-mono uppercase">Burnout Risk</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 font-mono">
                    <div className="p-3 bg-[#0c0c0e] rounded-xl border border-white/5">
                  ...
                  {/* Triggers identified */}
                  <div>
                    <h4 className="text-xs font-mono uppercase text-zinc-400 mb-2">Identified Core Triggers</h4>
                    <div className="flex flex-wrap gap-1.5">
                      {lastAnalysis.triggers.map((trig, idx) => (
                        <span 
                          key={idx}
                          className="bg-[#0c0c0e] px-2.5 py-1 text-xs rounded-lg border border-white/10 text-zinc-300"
                        >
                          🏷️ {trig}
                        </span>
                      ))}
                  ...
                  <div className="p-4 bg-white/5 rounded-xl border border-white/5 relative">
                    <span className="text-2xl text-indigo-500 font-serif absolute -top-1 left-2">“</span>
                    <p className="text-xs text-zinc-200 italic leading-relaxed pl-4 pr-2 font-sans pt-1">
                      {lastAnalysis.insights}
                    </p>
                  </div>

                  {lastAnalysis.crisisFlagged && (
                    <div className="flex gap-2 items-center text-red-400 bg-red-950/20 border border-red-900/30 rounded-xl p-3 text-xs leading-relaxed">
